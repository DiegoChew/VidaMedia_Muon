...
class paaFile:
    def __init__(self,FileName):
        ...
        # Define a counter for the iterator
    ...
    def __next__(self):
        self.current += 1
        if self.current < self.pc :
            return self.paaGetPulseAsNpArray( self.current )
        raise StopIteration
    ...
    def paaGetPulseAsNpArray(self,pulse_number):
        if( ( pulse_number < 0 ) or (pulse_number > (self.pc-1) )):
            print("Error: Bad pulse number")
            sys.exit()
        offset = 640 + 2*self.ps*(pulse_number)
        self.file.close() 
        pulse_data_raw = np.fromfile(self.filename, dtype=np.int16, count=self.ps, offset=offset)
        pulse_data = np.where( pulse_data_raw > 8191, pulse_data_raw - 16384, pulse_data_raw )
